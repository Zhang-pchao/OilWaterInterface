...
class VoronoiIMZ : public Colvar {
  bool pbc;
  bool serial;
  ...
  bool firsttime;
  int  lambda;
  int  zidx,nrx,num_atomsa,num_atomsb,num_atoms,num_atomso;
  double zmid,d0,d1,d2,d3,r0,sum_exp; 

public:
  explicit VoronoiIMZ(const ActionOptions&);
  ~VoronoiIMZ();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiIMZ,"VORONOIIMZ")

void VoronoiIMZ::registerKeywords( Keywords& keys ) {
  ...
  keys.add("compulsory","ZMID","0.0","The Z-axis coordinate of the slab center layer");
  keys.add("compulsory","ZIDX","0","The XYZ coordinate index: X=0,Y=1,Z=2");  
}

VoronoiIMZ::VoronoiIMZ(const ActionOptions&ao):
...
void VoronoiIMZ::calculate()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
	...
	std::vector<Vector> omp_ionz_deriv(getPositions().size());	
    ...
    #pragma omp for reduction(+:ion_z)
    for(unsigned int j=0; j<num_atomsa; j+=1) {
      ...
      //KEY CHANGE
      if(charge[j]<0.0 && j<num_atomso) {
		  ion_z+=std::abs(getPosition(j)[zidx]-zmid)*pow(charge[j],2);
		  omp_ionz_deriv[j][zidx]=pow(charge[j],2);
		  if((getPosition(j)[zidx]-zmid)>=0.0) {
		  	omp_deriv[j]+=omp_ionz_deriv[j];
		  } else {
		  	omp_deriv[j]-=omp_ionz_deriv[j];
		  }			  
	  }
    ...
    for(unsigned int i=0; i<nn; i+=1) {  //it is equivalent to the loop on n,j of Emanuele. The derivative will be evaulated on these two atoms
		...
		if (ind0 == k) d_in = 1;
		if (ind0 != k) d_in = 0;
		//ind0=Oi ind1=Hj k=Om
		fk = lambda * c[k][ind1] * (d_in - c[ind0][ind1]);
		
		if(charge[ind0]<0.0 && ind0<num_atomso) {
			buf=2.0*std::abs(getPosition(ind0)[zidx]-zmid)*charge[ind0]*fk;
		} else {
			buf=0.0;
		}	
