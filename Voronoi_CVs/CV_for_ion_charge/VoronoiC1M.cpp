...
class VoronoiC1M : public Colvar {
  bool pbc;
  bool serial;
...
public:
  explicit VoronoiC1M(const ActionOptions&);
  ~VoronoiC1M();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiC1M,"VORONOIC1M")

void VoronoiC1M::registerKeywords( Keywords& keys ) {
...
VoronoiC1M::VoronoiC1M(const ActionOptions&ao):
...
void VoronoiC1M::calculate()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    for(unsigned int j=0; j<num_atomso; j+=1) {    
	  ...
	  if(charge[j]<0.0) totcharge+=charge[j];  //for OH-    
    }    
