...
class VoronoiC1P : public Colvar {
  bool pbc;
  bool serial;
...
public:
  explicit VoronoiC1P(const ActionOptions&);
  ~VoronoiC1P();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiC1P,"VORONOIC1P")

void VoronoiC1P::registerKeywords( Keywords& keys ) {
...
VoronoiC1P::VoronoiC1P(const ActionOptions&ao):
...
void VoronoiC1P::calculate()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    for(unsigned int j=0; j<num_atomso; j+=1) {    
      ...
      if(charge[j]>0.0) totcharge+=charge[j]; //for H3O+     
    }    
    ...
    for(unsigned int i=0; i<nn; i+=1) {  //it is equivalent to the loop on n,j of Emanuele. The derivative will be evaulated on these two atoms
		...
		if(ind0 < num_atomso && charge[ind0] > 0.0) {
			buf=fk;
		} else {
			buf=0.0;
		}	
