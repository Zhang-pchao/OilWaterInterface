...
###CHANGE BELOW###
geo_path = "/work/pzhang/Glycine/geofile"
trj_path = '../'
save_path= './'
trj_name = os.path.join(trj_path,"glycine_10.lammpstrj")
geo_name = os.path.join(geo_path,"127w_1h3o_2.data")
cvs_file = plumed.read_as_pandas(os.path.join(trj_path,'COLVAR'))
ion_type = 'ip' # cv: 'im' for OH minus, 'ip' for H3O positive
tcs_type = 'tc' # cv: 'tc' for total change
o_type   = '3'  # original O type in .lammpstrj file
ionOtype = 9    # new type for O in Ion 
cvs_step = 1e1  # COLVAR file dump
