...
public:
  explicit VoronoiIP(const ActionOptions&);
  ~VoronoiIP();
// active methods:
...
PLUMED_REGISTER_ACTION(VoronoiIP,"VORONOIIP")

void VoronoiIP::registerKeywords( Keywords& keys ) {
...
VoronoiIP::VoronoiIP(const ActionOptions&ao):
...
void VoronoiIP::calculate()
{
  ...
  #pragma omp parallel num_threads(nt)
  {
    ...
    for(unsigned int j=0; j<num_atomsa; j+=1) {
      ...
      //KEY CHANGE
      if(charge[j]>0.0 && j<num_atomso) totcharge+=j*pow(charge[j],2);
    }

    //WARNING NO DERIVATIVE!!
